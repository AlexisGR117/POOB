...
public class DAPOOSTest {
    ...
    @Test
    public void deberiaAdicionarleUnaInvolnerabilidadAdicionalALafichaZombieElComodinInfinity() {
        try {
        	DAPOOS dapoos = new DAPOOS("Nombre Norte", "Nombre Sur", "Negro", "Blanco", false, false, 0, "Permanente");
        	Tablero tablero = dapoos.tablero();
    		Casilla casilla = tablero.casillas()[5][8];
    		casilla.colocarComodin(new Infinity(casilla));
    		tablero.casillas()[6][9].quitarFicha();
    		Ficha ninja = new Ninja(dapoos.jugadorSur());
    		tablero.casillas()[6][9].colocarFicha(ninja);
        	tablero.seleccionar(ninja);
        	...
        	tablero.seleccionar(tablero.ficha(5, 9));
        	tablero.moverSeleccion(6, 9);
        	tablero.seleccionar(ninja);
        	tablero.moverSeleccion(4, 7);
        	tablero.seleccionar(tablero.ficha(3, 6));
        	tablero.moverSeleccion(5, 8);
        	tablero.seleccionar(ninja);
        	tablero.moverSeleccion(4, 6);
        	assertNull(tablero.ficha(4, 6));
        	/*
            assertEquals(ninja, tablero.ficha(5, 8));
