...
/**
 * Tipo de Maquina que cunado tiene oportunidad de capturar lo hace, de lo contrario hace movimientos aleatorios.
...
public class Principiante extends Maquina {
	 ...
	 */
	public Principiante(String color, char lado) {
		super(color, lado);
	}
	...
	public void movimientoAutomatico() throws DAPOOSException {
		if (fichas.size() > 0) {
			...
			if (tablero.comodinActivo() instanceof Stomp) {
				...
				ArrayList<Object[]> pc = posiblesCapturas();
				Object[] movimiento;
				if (pc.size() > 0) movimiento = pc.get((int)(Math.random()*(pc.size())));
				else {
					ArrayList<Object[]> pm = posiblesMovimientos();
					movimiento = pm.get((int)(Math.random()*(pm.size())));
				}
